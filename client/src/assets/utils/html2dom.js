export const html2dom = (function() {
  "use strict";
  /*
  ...
  function dom2html(js, callback, errback) {
    ...
    if (_iframe == null) {
      errback(
        'This function requires this iframe attribute in the DOM: iframe id="iframe" src="data:text/html;charset=utf-8,<div id=\'container\'></div>" sandbox="allow-same-origin"></iframe>'
      );
    }
  ...
  function mkId(node) {
    ...
    Object.defineProperty(node, "h2d_nodeID", {
      configurable: true,
      writable: true
    }); // this looks like an awful hack. in fact...it is! :/
    if (name in ids) {
  ...
  function encodeForAttribute(s) {
    return s.replace(/[<>&'"/]/gi, function(c) {
  ...
  function newElement(node, el_name) {
    if (!("h2d_nodeID" in node)) {
      mkId(node);
    }
    if (el_name == "SCRIPT") {
      //XXX use a more generic way than this hard coded blacklist
      src += "//XXX CSP will forbid inline JavaScript!\n";
    }
    src += `\n${decl} ${node.h2d_nodeID} = document.createElement(${strToSrc(
      el_name.toLowerCase()
    )});\n`;
  }
  function newAttribute(node, attr, val) {
    ...
    if (attr == "style") {
      //XXX use a more generic way than this hard coded blacklist
      src +=
        "//XXX CSP will forbid inline styles. Use ``" +
        node.h2d_nodeID +
        ".style'' instead of setAttribute.\n";
    }
    val = encodeForAttribute(val);
    src += `${node.h2d_nodeID}.setAttribute(${strToSrc(attr)}, ${strToSrc(
      val
    )});\n`;
  }
  ...
  function newComment(node, cmt) {
    ...
    src += `${decl} ${node.h2d_nodeID} = document.createComment(${strToSrc(
      cmt
    )});\n`;
  }
  ...
  function walkNodes(root) {
    ...
    while ((node = iter.nextNode())) {
      let nodeDescr =
        node + ", name: " + node.nodeName + ", type: " + node.nodeType;
      ...
      if (node.nodeType == Node.ELEMENT_NODE) {
        ...
        if (parentName != undefined) {
          appendToParent(parentName, node);
        }
