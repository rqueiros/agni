...
const main = {
  ...
  getters: {
    ...
    getOccurrenceByClassID: state => id => {
      return state.occurrences
        .find(occ => occ.classes.map(c => c.id).includes(id))
    },
  ...
  actions: {
    ...
    createNewCollectionType(state, type){
      ...
      switch (type){
        case "course": this.dispatch("main/addCourse"); break;
        case "expositive": this.dispatch("main/addExpositive"); break;
        case "evaluative": this.dispatch("main/addEvaluative"); break;
        case "question": this.dispatch("main/addQuestion"); break;
        case "occurrence": this.dispatch("main/addOccurrence"); break;
      }
    },
    ...
    moveLesson(state, [id, direction]) {
      ...
      const indexModule = course.children.findIndex(obj => obj.id == module.id);
    ...
    addEvaluativeByLessonID2(state, [id, newExercise]){
      const lesson = state.getters.getLessonByID(id)
      let nEvaluatives = state.getters.getMinimumID(state.getters.getEvaluatives)
    ...
    setEvaluativeField(state, [id, field, value]) {
      let evaluative = state.getters.getEvaluativeByID(id)
      if (evaluative){
        if (evaluative[field] != value || field=="statement") this.commit("main/setChanged", true);
