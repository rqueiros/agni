...
const main = {
  ...
  getters: {
    ...
    getCourseByModuleID: state => id => {
      return state.courses.find(c => c.children.map(m => m.id).includes(id));
    },

    //-------Module
    getModuleByLessonID: state => id => {
    ...
    getModuleByID:state => id => {
      return state.courses[0].children.find(m => m.id == id);  
    },
    getModules: state => {
    ...
    getLessons: state => {
      return state.courses.flatMap(c => c.children).flatMap(m => m.children)
    },
    ...
    getStatusByResourceId: state => id => {
      return state.courses[0].children
        .flatMap(m => m.children)
        .flatMap(l => l.evaluatives)
  ...
  mutations: {
    ...
    addConditionByLMId(state, [id, type]) {
      ...
      if (type == "lesson") {
        ...
        element = state.courses.flatMap(c => c.children).find(m => m.id == id);
      }
      let n = Math.min(
        ...[
          ...state.courses
    ...
    addGPTExercises(state, [id, exercises]){
      const lesson = state.courses
        .flatMap(c => c.children)
        .flatMap(m => m.children)
        .find(l => l.id == id);
      ...
      exercises.forEach(exercise =>{
        ...
        exercise.tests.forEach(test => {
          test.new = true
          test.id = n2-countTests
          countTests++
        })
      })

      lesson.evaluatives.push(...exercises)
      this.commit("main/setChanged", true);
    },
      ...
      if (type == "quiz") {
        ...
        if (state.courses.length > 0) {
          ...
          if ("questions" in state.evaluatives[0]) {
            ...
            answers = state.evaluatives[0].questions.flatMap(q =>
    ...
    addAnswerByQuestionId(state, id) {
      ...
      if (state.courses.length > 0) {
        ...
        question = state.evaluatives[0].questions.find(q => q.id == id);
        answers = state.evaluatives[0].questions.flatMap(q =>
          q.answers.map(a => a.id)
        );
      } else {
    ...
    deleteAnswer(state, id) {
      ...
      if (state.courses.length > 0) {
        ...
        question = state.evaluatives[0].questions.find(q =>
    ...
    setOccurrenceField(state, [id, field, value]) {
      ...
      occ[field] = value;
      if (field=="startDate"){
        occ.year = Number(value.substring(0,4))
      }
    },
    ...
    addClassByOccurrenceId2(state, [id, classe]) {
      ...
      const students = state.occurrences
        .flatMap(o => o.classes)
      ...
      classe.students.forEach(student => {
        (student.new = true), (student.id = n2 - c2);
        c2++;
      });
    ...
    deleteClass(state, id) {
      const occ = state.occurrences.find(o =>
        o.classes.map(o => o.id).includes(id)
      );
    ...
    addStudentByClassName(state, [name, students]) {
      const classe = state.occurrences
        .flatMap(o => o.classes)
        .find(c => c.name == name);
      const allStudents = state.occurrences
        .flatMap(o => o.classes)
        .flatMap(c => c.students)
        .map(c => c.id);
    ...
    setStudentField(state, [id, field, value]) {
      const student = state.occurrences
        .flatMap(o => o.classes)
        .flatMap(c => c.students)
        .find(s => s.id == id);
    ...
    deleteStudent(state, id) {
      const classe = state.occurrences
        .flatMap(o => o.classes)
        .find(c => c.students.map(s => s.id).includes(id));
    ...
    setStatusField(state, [id, field, value]) {
      const status = state.courses.flatMap(c=>c.children).flatMap(m=>m.children).flatMap(l=>l.evaluatives).find(e => e.id == id).status
      status[field] = value;
    },
    ...
    editableInput(state, obj) {
      ...
      const id = obj.id;
      if (type == "course") {
        this.dispatch("main/setCourseField", [id, field, value]);
      } else if (type == "module") {
        this.dispatch("main/setModuleField", [id, field, value]);
      } else if (type == "lesson") {
        this.dispatch("main/setLessonField", [id, field, value]);
        ...
        this.commit("main/setStatusField", [id, field, value]);
      } else if (type == "user") {
  ...
  actions: {
    createNewCollectionType(state, type){
      ...
      this.commit("main/setRole", "author");
      switch (type){
        case "course": state.dispatch("addCourse"); break;
      }
    },
    //-------Course
    ...
    async addCourse(state) {
      const courses = state.getters.getCourses
      ...
      course = await state.dispatch("makeCourseEditable", course)
      courses.push(course)
    ...
    async fetchCourse(state) {
      ...
      const course = await this.dispatch("convert/prepareCourse", [
        resp,
        false
      ]);
    ...
    async copyCollectionType(state, [id, collectionType]) {
      this.commit("main/deleteStructure");
      this.commit("main/setChanged", true);
      let resp = await state.dispatch("fetchCollectionType", [
        id,
        collectionType
      ]);
    ...
    async prepareExpositive(state, [resp, isCopy]) {
      const expositive = await this.dispatch("convert/prepareExpositive", [
        resp,
        isCopy
      ]);
      this.commit("main/setExpositives", [expositive]);
    ...
    async prepareEvaluative(state, [resp, isCopy]) {
      const evaluative = await this.dispatch("convert/prepareEvaluative", [
        resp,
        isCopy
      ]);
      this.commit("main/setEvaluatives", [evaluative]);
    ...
    async prepareQuestion(state, [resp, isCopy]) {
      const question = await this.dispatch("convert/prepareQuestion", [
        resp,
        isCopy
      ]);
      this.commit("main/setQuestions", [question]);
    ...
    async prepareOccurrence(state, [resp, isCopy]) {
      const occurrence = await this.dispatch("convert/prepareOccurrence", [
        resp,
        isCopy
      ]);
    ...
    async saveCollectionType(state, collectionType) {
      ...
      if (collectionType == "courses") {
        [data, files] = await this.dispatch(
          "convert/prepareCourseForServer",
          data
        );
      } else if (collectionType == "occurrences") {
        [data, files] = await this.dispatch(
          "convert/prepareOccForServer",
          data
        );
      } else if (collectionType == "expositives") {
        [data, files] = await this.dispatch(
      ...
      await this.dispatch("main/fetchPrepareCollectionType", [
        id,
        collectionType
      ]);
    },
    ...
    async addExistingExpositives(state, [lessonId, expositveIds]) {
      ...
      for (let expositveId of expositveIds) {
        let expositive = await this.dispatch("main/fetchCollectionType", [
          expositveId,
          "expositives"
        ]);
        expositive = await this.dispatch("convert/prepareExpositive", [
          expositive,
          false
        ]);
        let lesson = state.getters.getCourse.children
          .flatMap(m => m.children)
    ...
    async addExistingEvaluatives(state, [lessonId, evalutaiveIds]) {
      ...
      for (let evaluativeId of evalutaiveIds) {
        let evaluative = await this.dispatch("main/fetchCollectionType", [
          evaluativeId,
          "evaluatives"
        ]);
        evaluative = await this.dispatch("convert/prepareEvaluative", [
          evaluative,
          false
        ]);
        let lesson = state.getters.getCourse.children
    ...
    async addExistingQuestions(state, [evaluativeId, questionIds]) {
      ...
      for (let questionId of questionIds) {
        let question = await this.dispatch("main/fetchCollectionType", [
          questionId,
          "questions"
        ]);
        question = await this.dispatch("convert/prepareQuestion", [
          question,
          false
        ]);
        let evaluative;
        if (state.getters.getCourses.length > 0) {
          evaluative = state.getters.getCourse.children
            .flatMap(m => m.children)
            .flatMap(l => l.evaluatives)
            .find(e => e.id == evaluativeId);
        } else {
    ...
    async generateProgrammingEx(state, [messageResp, id]) {
      const auth = "Bearer " + state.getters.getJWT;
