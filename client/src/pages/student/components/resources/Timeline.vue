<template>
  <div id="timeline" style="width:100%">
    <v-card class="mx-auto" outlined v-if="resource != undefined && resExist" id="timeline">
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title :class="getTitleClass(screenSize)"> TIMELINE </v-list-item-title>
          <v-list-item-subtitle :class="getSmallTextClass(screenSize)" v-if="'type' in resource">Jump in the {{
            resource.type.toUpperCase() }} for specific
            topics!</v-list-item-subtitle>
        </v-list-item-content>

        <v-list-item-avatar tile :size="getAvatarMediumSize(screenSize)" color="green">
      ...
      <v-timeline align-top dense v-if="('milestones' in resource && resource.milestones.length != 0) || isTeacher"
        ...
        <v-timeline-item class="milestone" @click.native="goto(parseInt(milestone.frame))" :icon="getIcon(milestone)"
          ...
          <v-row v-if="isStudent || isViewer">
            <v-col class="mt-1">
              <span v-html="convert(milestone.frame) + ' - ' + milestone.label"></span>
            </v-col>
          </v-row>
          <!--Author-->
          <v-row v-if="isAuthor" class="ml-0">
            <v-col class="mt-1 pt-3" style="padding:2px;" cols="2">
              <Editable :type="'milestone'" :value="milestone.frame" :id="milestone.strapiId" :field="'frame'"
                placeholder="Frame" @input="editableInput" onclick="event.stopPropagation()"></Editable>
            </v-col>
            <v-col class="mt-1 pt-3" style="padding:2px;">
              <Editable :type="'milestone'" :value="milestone.label" :id="milestone.strapiId" :field="'label'"
                placeholder="Label" @input="editableInput" onclick="event.stopPropagation()"></Editable>
            </v-col>
            <v-col class="pt-3" style="padding:2px;">
              <v-btn icon @click="deleteMilestone(milestone.strapiId)" class="course_iconButtonS">
        ...
        <v-timeline-item hide-dot v-if="isAuthor">
          <v-btn @click="addMilestoneByExpositiveId(resource.strapiId)" small>
...
export default {
  ...
  computed: {
    ...mapGetters(["getRole", "getTitleClass", "getSmallTextClass",
      "getAvatarMediumSize", "getIconBigSize", "getIconSmallSize"]),
    resExist() {
