<template>
    <div id="contexts">
        <v-card class="mx-auto" max-width="100%" outlined v-if="resource.contentType == 'new'"
            ...
            <v-row style="margin:0;height:15rem; display: flex; justify-content: center; align-items: center; padding:1em">
                <v-col cols="6">
                    <v-file-input class="course_smallText course_button" label="File input" v-model="file" prepend-icon=""
                        disabled outlined></v-file-input>
                </v-col>
                <v-col cols="6">
                    <v-btn class="course_smallText course_button" outlined @click="changeContext">Write File</v-btn>
                </v-col>
            </v-row>
        </v-card>
        <v-card class="mx-auto" max-width="100%" outlined v-else
            style="display: flex; justify-content: center; align-items: center;">
            <AceEditor ref="skeleton" v-model="code" @init="editorInit" lang="javascript" class="course_text"
                theme="ambiance" width="100%" height="15em" :options="editorOp" :commands="com" />
        </v-card>
        ...
        file: function (newFile, oldFile) {
            if (newFile !== oldFile) {
                if (newFile != [] && newFile != undefined) {
                    this.changeContextTypeById(this.resource.strapiId)
                }
            }
        },
        'resource.strapiId'(_,oldId){
            console.log(oldId)
