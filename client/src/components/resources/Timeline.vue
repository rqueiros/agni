  ...
  <v-card class="mx-auto" outlined>
    ...
    <v-card-text>
      ...
      <v-subheader v-else
        >PDF length:
        {{
          resource.milestones.find((milestone) => milestone.label === "End")
            .index
        }}
        pages
      </v-subheader>
      ...
      <v-timeline align-top dense v-if="resource.milestones.length!=0">
        <v-timeline-item
          class="milestone"
          @click.native="goto(milestone.index)"
        ...
        >
          <v-row>
            <v-col class="mt-1">
              <strong
                class="subtitle-1"
                v-html="convert(milestone.index) + ' - ' + milestone.label"
              ></strong>
            </v-col>
          </v-row>
        </v-timeline-item>
...
export default {
  ...
  methods: {
    ...
    getColor(milestone) {
      let color;
      if (milestone.index == this.selected) {
        color = "black";
      } else {
